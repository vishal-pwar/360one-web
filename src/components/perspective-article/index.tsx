    ...
    <section>
      <div className="flex flex-col gap-14 bg-black pt-40 px-72">
        <div className="flex justify-between">
          <div>
            ...
            <div className="font-hanken-grotesk font-semibold text-base text-white">
              {articleResponse?.article?.data?.attributes?.date}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="font-medium text-[22px] text-white my-1">
              {articleResponse?.article?.data?.attributes?.written_by}
            </div>
            <div className="font-normal text-sm text-white">
              {articleResponse?.article?.data?.attributes?.designation}
            </div>
            <div className="font-normal text-sm text-white">
              {articleResponse?.article?.data?.attributes?.company_name}
            </div>
          </div>
        </div>
        <div className="flex">
          {articleResponse?.category?.length > 0 ? (
            <div className="absolute mt-14 text-lg font-bold text-white bg-[#FD7740] py-3 px-6">
              {articleResponse?.category}
            </div>
          ) : null}
          {articleResponse?.media?.data?.attributes?.url?.length > 0 ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img
              src={articleResponse?.media?.data?.attributes?.url}
              alt={
                articleResponse?.media?.data?.attributes?.url?.alternativeText
              }
      ...
      <div className="bg-[#F8F8F8]">
        <div className="bg-white mx-72 p-10">
          {articleResponse?.article?.data?.attributes?.read_time?.length > 0 ? (
            <span className="w-48 mb-6 flex flex-row font-hanken-grotesk font-semibold text-base text-[#B1532D] px-4 py-2 bg-[#FFE4D9]">
              <span>Read time - </span>
              <span className="mx-1">
                {articleResponse?.article?.data?.attributes?.read_time}
              </span>
              <span>mins</span>
            </span>
          ) : null}
          {articleResponse?.article?.data?.attributes?.card?.map((i: any) => {
            return (
              <>
